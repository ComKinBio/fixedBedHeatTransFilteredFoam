...
void Foam::WallSpringSliderDashpotHeat<CloudType>::findMinMaxProperties
(
    scalar& rMin,
    scalar& rhoMax,
    scalar& UMagMax
...
template<class CloudType>
void Foam::WallSpringSliderDashpotHeat<CloudType>::evaluateWall
(
...
template<class CloudType>
void Foam::WallSpringSliderDashpotHeat<CloudType>::evaluateWallHeat
(
    typename CloudType::parcelType& p,
    const point& site,
    const WallSiteData<vector>& data,
...
{
    const volScalarField& T = this->owner().T();
    
    //- TODO now only fixed value and is applied, 
    //- if the boundary type is not fixed value then treated as zore gradient
    
    if(T.boundaryField()[data.patchIndex()].type()=="fixedValue"  || T.boundaryField()[data.patchIndex()].type()=="uniformFixedValue")
    {
        scalar T_wall = T.boundaryField()[data.patchIndex()][0];
        ...
        if (cohesion)
        {
            if (normalOverlapMag > 0.0)
            {
                // heat conduction
                scalar dT_PW = (T_wall - p.T());
                
                scalar k_p = p.kpp(p.T());
                
                //radius of the contact area of two contacting particles
                scalar r_c_wall = max(0, sqrt(mag(sqr(pREff) - sqr(r_PW_mag))));
                
                //Batchelor and O'Brian 1977.  Zhou and Yu AIChE 2009, eq 16
                scalar Q_WalltoP = (4 * r_c_wall * dT_PW) / (1/k_p + 1/k_);
                
                p.dT() += Q_WalltoP;
                
                
                // radiation, here contact to wall is treated as contact to a particle
                //update partice contacted neighbour number
                p.neighbourNum() += 1.0;
        
                //update partice contacted T4
                p.ppT4sum() += Foam::pow4(T_wall);
...
template<class CloudType>
Foam::WallSpringSliderDashpotHeat<CloudType>::WallSpringSliderDashpotHeat
(
...
template<class CloudType>
Foam::scalar Foam::WallSpringSliderDashpotHeat<CloudType>::pREff
(
...
template<class CloudType>
Foam::scalar Foam::WallSpringSliderDashpotHeat<CloudType>::pREffBeforeDEM
(
    const typename CloudType::parcelType& p
) const
{
    if (useEquivalentSize_)
    {
        return p.dBeforeDEM()/2*cbrt(p.nParticle()*volumeFactor_);
    }
    else
    {
        return p.dBeforeDEM()/2;
    }
}


template<class CloudType>
bool Foam::WallSpringSliderDashpotHeat<CloudType>::controlsTimestep() const
{
    return true;
}
...
template<class CloudType>
Foam::label Foam::WallSpringSliderDashpotHeat<CloudType>::nSubCycles() const
{

// std::cout<<"on processor: "<<Pstream::myProcNo()<<' '<<" wallSubCycles.nSubCycles() enter"<<'\n';
    ...
    //in specified case the particle UMagMax is 0 
    //then (Estar_*sqrt(UMagMax) + vSmall) = vSmall which is 2.225073859e-308
    //rhoMax/(Estar_*sqrt(UMagMax) will cause floating point exception
    //so a check fro UMagMax is made
    scalar MinUMagMax = pow((rhoMax/(Estar_*pow((this->owner().time().deltaTValue()/rMin/5.429675),2.5))), 2);
    
    scalar minCollisionDeltaT = this->owner().time().deltaTValue() + vSmall;
    
    if (UMagMax > MinUMagMax)
    {
        minCollisionDeltaT =
        5.429675
       *rMin
...
{
    scalar pREff = this->pREffBeforeDEM(p);
