...
template<class CloudType>
void Foam::WallSpringSliderDashpot<CloudType>::evaluateWallHeat
(
    typename CloudType::parcelType& p,
    const List<point>& flatSitePoints,
    const List<WallSiteData<vector>>& flatSiteData,
...
{
    Info<< "wrong model is used!!!"<< nl << endl;
}


// ************************************************************************* //
// if(std::isnan(p.d()) || std::isnan(p.rho())|| std::isnan(mag(p.U()))|| std::isnan(mag(p.omega()))|| std::isnan(p.nParticle()))
// {
//     std::cout<<"processor: "<<Pstream::myProcNo()<<' '<<"particle variables have nan"<<'\n';
//     std::cout<<"partice A number is : "<< p.nParticle()<<' '<<"diameter is : "<<dEff <<' '<<"velocity is : "<<mag(p.U()) <<' '<<"density is : "<<p.rho() <<' '<<"omega is : "<<mag(p.omega()) <<' '<<'\n';
// }
//         if (notNull(iter))
//         {
//             std::cout<<"on processor: "<<Pstream::myProcNo()<<" partice iter is real ptr "<<'\n';
//         }
//         else
//         {
//             std::cout<<"on processor: "<<Pstream::myProcNo()<<" partice iter is nullptr "<<'\n';
//         }
//      std::cout<<"on processor: "<<Pstream::myProcNo()<<" processor partice number is : "<< this->owner().size()<<'\n';
//     std::cout<<"on processor: "<<Pstream::myProcNo()<<" partice number is : "<< p.nParticle()<<' '<<"diameter is : "<<p.d() <<' '<<"velocity is : "<<mag(p.U()) <<' '<<"density is : "<<p.rho() <<' '<<"omega is : "<<mag(p.omega()) <<' '<<'\n';
//     std::cout<<"on processor: "<<Pstream::myProcNo()<<"velocity is : "<<"("<<p.U().x()<<","<<p.U().y() <<","<<p.U().z()<<")"<<' '<<"omega is : "<<"("<<p.omega().x()<<","<<p.omega().y() <<","<<p.omega().z()<<")"<<'\n';
