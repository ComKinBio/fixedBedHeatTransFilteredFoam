...
(
    30*sizeof(scalar) + 2*sizeof(label)
);

// template<class ParcelType>
// const std::size_t Foam::ReactingMultiphaseParcel<ParcelType>::sizeofFields_
// (
//     232 //26*sizeof(scalar=8) + 3*sizeof(label=4) + 3*sizeof(scalarField=24)
//         //-(YGas_(0) + YLiquid_(0)+ YSolid_(0))((scalarField=24)) - canCombust_(0)(label=4)- (Qin_(0.0) + QLayer_(0.0))(scalar=8) = 25*sizeof(scalar=8) +4VID(scalar=8)
// //   176
// //     sizeof(ReactingMultiphaseParcel<ParcelType>) - offsetof(ReactingMultiphaseParcel<ParcelType>, Tb0_)
// //     offsetof(ReactingMultiphaseParcel<ParcelType>, cumTime_)
// //     -offsetof(ReactingMultiphaseParcel<ParcelType>, Tb0_)
// //     +sizeof(scalar)
// );
    ...
    if (readFields)
    {
        ...
        {   
            ...
            is >>  Tb0_ >> Tb1_ >> Tb2_ >> Tb3_ >> rb0_ >> rb1_ >> rb2_ >> rb3_ 
               ...
               >> rDry_ >> rDevo_ >> rChar_ >> rComb_ >> ash_inchar_t_ >> QDry_ >> QComb_ >> flagBoiling_ >> flagDevo_ 
               >> cumTime_ >> Qin_ >> QLayer_ >> VID1_ >> VID2_ >> VID3_ >> VID4_ >> Yg >> Yl >> Ys;
        }
        else
        {
...
)
{
    ...
    IOField<scalar> Qin(c.fieldIOobject("Qin", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, Qin); 
    
    IOField<scalar> QLayer(c.fieldIOobject("QLayer", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, QLayer); 
    
    IOField<scalar> VID1(c.fieldIOobject("VID1", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, VID1); 
    
    IOField<scalar> VID2(c.fieldIOobject("VID2", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, VID2); 
    
    IOField<scalar> VID3(c.fieldIOobject("VID3", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, VID3); 
    
    IOField<scalar> VID4(c.fieldIOobject("VID4", IOobject::MUST_READ), valid);
    c.checkFieldIOobject(c, VID4); 
...
)
{
    ...
    // Write the composition fractions
    {
        ...
        IOField<scalar> Qin(c.fieldIOobject("Qin", IOobject::NO_READ), np);
        IOField<scalar> QLayer(c.fieldIOobject("QLayer", IOobject::NO_READ), np);
        IOField<scalar> VID1(c.fieldIOobject("VID1", IOobject::NO_READ), np);
        IOField<scalar> VID2(c.fieldIOobject("VID2", IOobject::NO_READ), np);
        IOField<scalar> VID3(c.fieldIOobject("VID3", IOobject::NO_READ), np);
        IOField<scalar> VID4(c.fieldIOobject("VID4", IOobject::NO_READ), np);

        label i = 0;
        
        forAllConstIter(typename Cloud<ReactingMultiphaseParcel<ParcelType> >, c, iter)
        {
