...
)
{
    ...
    {
        td.rhoc() = td.rhoavgInterp().interpolate(this->coordinates(), tetIs);
        ...
        td.Uc() = td.UavgInterp().interpolate(this->coordinates(), tetIs);

        td.muc() = td.muavgInterp().interpolate(this->coordinates(), tetIs);
        
        td.alphac() = td.alphaavgInterp().interpolate(this->coordinates(), tetIs);
    }
    else
    {
...
template<class ParcelType>
template<class TrackCloudType>
bool Foam::KinematicParcel<ParcelType>::calcParticle
(
    TrackCloudType& cloud,
    trackingData& td,
...
)
{
    ...
    if (trackTime > rootVSmall)
    {
        ...
        p.calcDispersion(cloud, ttd, trackTime);

        if (cloud.solution().cellValueSourceCorrection())
        {
            p.cellValueSourceCorrection(cloud, ttd, trackTime);
        }
        
        p.calc(cloud, ttd, trackTime);
    }
...
//         Info<<"dt at move functions: "<<dt<<endl;
//         Info<<"trackTime at move functions: "<<trackTime<<endl;
//         Info<<"p.stepFraction() - sfrac at move functions: "<<(p.stepFraction() - sfrac)<<endl;
