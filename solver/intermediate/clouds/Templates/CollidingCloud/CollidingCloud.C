...
void Foam::CollidingCloud<CloudType>::calcParticleCollide
(
...
)
{
    ...
    this->collision().collideHeat();
    
    CloudType::calcParticle(cloud, td, deltaT);
}
...
template<class CloudType>
template<class TrackCloudType>
void  Foam::CollidingCloud<CloudType>::conversion
(
    TrackCloudType& cloud,
    typename parcelType::trackingData& td
)
{
    if (cloud.DEMFlag == 1)
    {
        ...
        {
            typename TrackCloudType::parcelType& p = iter();
    ...
    calcParticleCollide(cloud, td, this->db().time().deltaTValue());
