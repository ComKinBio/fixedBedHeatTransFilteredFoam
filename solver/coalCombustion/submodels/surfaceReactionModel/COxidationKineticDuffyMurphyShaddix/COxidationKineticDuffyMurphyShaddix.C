...
Foam::label Foam::COxidationKineticDuffyMurphyShaddix<CloudType>::maxIters_ = 200;

template<class CloudType>
Foam::scalar Foam::COxidationKineticDuffyMurphyShaddix<CloudType>::tolerance_ = 1e-06;

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

template<class CloudType>
Foam::COxidationKineticDuffyMurphyShaddix<CloudType>::
...
COxidationKineticDuffyMurphyShaddix
(
    const COxidationKineticDuffyMurphyShaddix<CloudType>& srm
)
:
...
template<class CloudType>
Foam::scalar Foam::COxidationKineticDuffyMurphyShaddix<CloudType>::calculate
(
    const scalar dt,
    const label celli,
    const scalar d,
...
//    Info<<"flag 8-----------------------------------"<<endl;    
    // Kinetic rate beta_r
    const scalar beta_r_O2 = C2_1_*exp(-E_1_/T_adjust);
    const scalar beta_r_CO2 = C2_2_*exp(-E_2_/T_adjust);
    const scalar beta_r_H2O = C2_3_*exp(-E_3_/T_adjust);
    const scalar beta_r_H2 = C2_4_*exp(-E_4_/T_adjust);
    
    scalar qCsO2, qCsCO2, qCsH2O, qCsH2;
    scalar qCsOld = 0;
...
// }
 ...
 dHeatSRCarrier[7] = (hmi_O2*beta_r_O2/(hmi_O2+beta_r_O2));  
            ...
            {
                WarningInFunction
                << "YO2Surface is negative value" << nl << endl;
            }
//         
        if (iter > maxIters_)
        {
...
/*if (T>1500)
{
     Info<<"diffusion end: ========================================="<<endl; 
}
  */       
    dHeatSRCarrier[7] = (beta_r_O2*beta_d_O2/(beta_r_O2+beta_d_O2));  
    }
    dHeatSRCarrier[4] = T_adjust;//beta_r_O2;
    
    
//     dHeatSRCarrier[7] = hmi_O2;
    /*const scalar D0_CO2 = C1_CO2_*pow(0.5*(T_adjust + Tc), 1.81);
    const scalar D0ash_CO2 = C1_CO2_*pow(T_adjust, 1.81);
    const scalar D0_H2O = C1_H2O_*pow(0.5*(T_adjust + Tc), 1.81);
    const scalar D0ash_H2O = C1_H2O_*pow(T_adjust, 1.81);
    const scalar D0_H2 = C1_H2_*pow(0.5*(T_adjust + Tc), 1.81);
    const scalar D0ash_H2 = C1_H2_*pow(T_adjust, 1.81);*/
...
//  Info<<"dmC:   "<<dmC<<"WC_:   "<<WC_<<"omega_c:   "<<omega_c<<"dMassSolid[CsLocalId_]:   "<<dMassSolid[CsLocalId_]<<endl; 
...
/*    if (T > 1000.0 || T < 0.0)
{
    ...
    {
        WarningInFunction
                    << "oxydization rate is negative value" << nl << endl;
    }
